...
- `golem/studio/tactics_play/` = 외형(검증 엔진을 **읽기전용** require, 룰 복제 안 함). `index.html`(`gen_tactics_play.py`=턴 재생 뷰어) · **`play.html`(`gen_tactics_interactive.py`=직접 플레이, 픽셀 스프라이트)** · `levels.json`(`propose_levels.py` 생성 8레벨) · `assets/tile_sprites.json`(Gemma 비전 선택 픽셀, CC0 Kenney). 수정은 `tactics_base_*`/제너레이터에서만.
...
- **레벨 시스템**: `propose_levels.py`(골렘 레벨 생성) + `play_signals.py`(결정적 신호=풀이/최소턴/지배전략/카드영향, 키0 검증·"실노출" 칸) → 변별 커브 `levels.json`.
- **비주얼**: `sprites.py`(SVG 폴백) / `gen_assets.py`(CC0 픽셀팩→컨택트시트→Gemma 비전 선택→tile_sprites.json). 인터랙티브 플레이 `gen_tactics_interactive.py`.
