...
- **핵심 도구(live)**: `build_graded.py`·`patch_apply.py`·`graft.py`·`card_delta.py`·`driver_autocard.py`·`propose_cards.py`·`gen_tactics_story.py`·`gen_tactics_play.py`.
- **루트 `golem/` live 의존(건드리지 말 것)**: `driver.py`·`static_gate.py`·`oracle.py`·`game_bank.py`·`grade.py`·`worker_prompt.py`·`bank_init.py` (build_graded/driver가 import).
- **과거 실험(legacy, 본선 무관·미사용)**: `golem/bank_add_*`·`oracle_design*`·`promote_solution.py`·`make_golden.py`·`design_probe.py`·`key_probe.py`·`demo_part.py`·`campaign.py`·`bank_remodularize_p3.py`. *(driver.py 디커플링 후 `legacy/`로 물리 이동 예정 — 지금은 driver.py가 일부를 import해 이동 시 본선이 깨짐.)*
