...
def main(argv=None):
    ...
    _pack = PLAY / "assets" / "tile_sprites.json"
    if _pack.exists():
...
<style>
  ...
  *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--txt);font:14px/1.4 system-ui,'Segoe UI',sans-serif}
  ...
  .stage{display:flex;gap:14px;flex-wrap:wrap}canvas{background:rgba(44,52,86,0.82);border-radius:12px;touch-action:manipulation}
