...
function applyAction(state, action) {
  ...
  if (action.type === 'move') {
    ...
    const range = (ally.range && ally.range > 0) ? ally.range : 1;
    const target = state.enemies
      .filter(e => {
        const dist = manhattan(ally.pos, e.pos);
        return e.hp > 0 && dist >= 1 && dist <= range;
      })
...
function executeEnemyAI(state, enemy) {
  ...
  // Target ally: min distance, tie-break by min ID
  ...
  if (distToTarget === 1) {
    ...
    if (candidates.length > 0) {
      candidates.sort((a, b) => {
        ...
        // X-axis reduction > Y-axis reduction
...
function updateState(state, action) {
  ...
  // Immediate check after ally action
