...
- [~] **확장 카드 A 재측정 시도(G73 후속)**: graded-232623 attempt03 소켓 무한행(llm.py 타임아웃 부재)→킬. 건강 콜 ~11분·~20k 출력토큰=출력바운드. **A 전체주입 측정 보류**(자기모순적 확장).
...
- [ ] **(다음) B 중심 측정 라인**: 확장 카드(폐기 아님, 선택적 읽기 스트레서)로 B만 ★키 측정·출력크기 추적. + llm.py 1800초 타임아웃 안전망. + 패치빌드 라인 §21 정본 정합. 곁=combat 자율oracle / 외부리뷰 P1(#10)
