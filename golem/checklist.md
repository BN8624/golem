...
- [ ] **소설→게임 브리지 첫 실증**: 아뜰리에 "에테르노의 그림자"(12장면) → 내러티브 IF 뼈대 + 턴제 전투 카드 하이브리드. 첫 카드=검문 잠입+조각1 각성(IF), 이후 변칙검술 전투(combat_base) 누적. 골렘=구조화+건전성검증, 각색충실도=사람. (아뜰리에 읽기전용.)
- [ ] **싼 사전필터 조각 ① 착수**: `card_proposer.py`(B+검토) 위에 "될 것 같은 top N" 신호층 + 검토교훈/좋은예시 레저(RAG, 사람 검토를 코드로 흡수). 모델-비평가=불완전한 취향 대리로 명시.
