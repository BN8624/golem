...
비대상 = NPC 자유대화·탐험 서사 어드벤처(정확일치 채점 불가, Golem 검증 모델 밖).

**외형 저작 역할(2026-06-23, Godot 트랙 정합 — 옛 "아트·음악·UI 폼은 사람 몫"을 대체).** 충돌 해소.
- 아트 원본 에셋의 직접 제작·수집·라이선스 확인 = 사람 또는 Claude가 준비할 수 있다(래스터 픽셀아트 생성은 현 모델 천장 밖).
- 준비된 에셋을 배치하고 맵·화면·UI·이펙트를 구성하는 **저작 = GOLEM**(씬·`_draw`·정책 전부 ★키). [[golem-does-the-art-too]]
- 사양·계약·골든·실행 하네스·증거 수집 = Claude(최종 화면을 직접 완성·고정하지 않고, 오류를 계약으로 되돌린다).
- 최종 미관·조작감·재미 판정 = 사용자. 자동 검증은 "올바르게 실행/룰 일치/입력 후 상태 변경/웹 로드"만 책임지고 재미를 통과했다고 주장하지 않는다.
