...
2. **편집 모드** — "기존 파일 + 이번 카드 = 수정본/diff"를 내게 한다(지금은 scratch 생성만).
3. **누적 회귀** — 카드 N에서 카드 1..N 시나리오 골든을 다 재실행(이전 기능 안 깨졌나).
4. **선택적 컨텍스트** — 게임이 커지면 전체가 프롬프트에 안 들어간다. "이번 카드가 건드리는 모듈만" 주입.
   **이게 큰 게임의 진짜 천장**이고, Golem의 계약/모듈 분해가 그걸 가능케 하는 토대다.
...
- **컨텍스트 천장** — 코드가 커서 프롬프트에 안 들어감. 해결 = 모듈 분해 + 인터페이스 계약 + 선택적 컨텍스트
  (21.2-④). 천장은 "큰 걸 못 만든다"가 아니라 "잘 설계해야 만든다"로 옮겨간다 — 대형 SW가 이미 쓰는 답이다.
