...
**현재 상태 (2026-06-18, G73)**: 누적 빌드 4레버 전부 닫힘 + 외부리뷰 반영 + CI 그린. 본선은 **31solo**(gemma-4 31B 단독). 스케일 확장 1차 A/B ★키 둘 다 1.0(천장 ~14.6k 미포착) → 카드 1.77x(63.5KB·46모듈) 확장 → A 재측정 소켓행→킬·사후채집 A @ 63.5KB = 1.0(충실도 드리프트) → 출력 32k 재프레이밍 → **확장 카드 B ★키 측정 = 게이트 11/11·합의 1.0·golden_diff 0·held-out 드리프트 0(B 우위 실증).** 레버4 천장 46모듈/~26k에서도 미포착.
...
**▶▶ 다음 세션 첫 동작 = B를 더 큰 카드로 밀어 *선택적 읽기* 천장 찾기 (방향 전환 후속)**.
...
- 핵심 미해결: B의 출력은 게임크기와 분리(항상 작음)지만, **입력(held-out 시그니처)은 모듈 수에 비례해 커짐** → 진짜 천장은 *시그니처 수가 많아질 때 모델이 묻힌 engine을 제대로 읽나*(입력 lost-in-the-middle). 46모듈/~26k에선 미포착. **다음: 카드를 시그니처 다수(예 100+모듈/입력 60~100k)로 키워 B만 재측정** — 출력은 여전히 ~수백 토큰이라 싸고 행 위험 작음.
- 인프라(완료): `llm.py` per-request 타임아웃 안전망 추가(`REQUEST_TIMEOUT_MS=1800000` 30분, 좀비 소켓→transient 백오프). 커밋 466a5cb. 키0 검증·run_keyless ALL PASS.
- 패치빌드 정합: 파일별/diff 빌드 라인이 §21 대형게임 정본과 어떻게 붙는지 정리.
...
1. **(다음 세션 첫 동작) B(패치) 중심 측정으로 전환** — A 전체주입은 출력바운드(자기모순적 확장)라 보류. 확장 카드(63.5KB·46모듈)로 **B만** ★키 측정(콜≈6k, `--inject-modules src/engine.js`)해 선택적 읽기 스트레서에서 정확도·출력크기 추적. + 인프라: `llm.py` 1800초 타임아웃 안전망 검토. 상세는 위 "핵심 재프레이밍" 참조.
