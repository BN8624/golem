...
- **지금 할 일 한 줄**: 두 트랙 진전. **(A) 정량 트랙 종결(G53~58)**: 합의를 정하는 건 결합도 아니라 *계약 빡빡함, 단 규칙 종류 의존*(종료조항 올림 d≈3 4회재현 / PHASE 순서 안 올림 / 결합도 가설 기각 eco0.925 vs combat0.633). **(B) 자율 oracle frontier(G60~63) — 현재 라이브 스레드**: 31B가 빌드 0줄로 골든을 자율 생성 → 방치형 0.879, *어휘 한 줄 박으니 1.0 수렴*(G61), 고결합 eco도 결정적 계산 7/8 무결(G62)→어휘+번식 박으니 **1.0 완전수렴**(G63, idle 동형). **핵심 = 결합도는 계산 안 깨뜨림(저/고 무관 1.0 수렴), 실패는 항상 계약 모호(enum 어휘·번식 규칙)이고 박으면 닫힘 → 자율 oracle은 viable + self-correcting + 공짜 모호탐지기(원문↔골든 계약버그까지 탐지, G63).** 도구 `studio/auto_oracle.py`(키별 `key_accuracy_by_name` 봐야 함, 시나리오단위 0.0은 아티팩트). **다음(★키) = ① reconcile 입력에 자율oracle 배선(손-oracle 대체) ② 31B self-suggest(자기 불일치→어느 계약줄 박을지) ③ combat 자율oracle. 곁 = eco·combat 잔차 / 외부리뷰 P1(#6·#4·#5·#10)**.
...
13. **(다음, ★키) 자율 oracle frontier 잔여**: ① reconcile 입력에 자율 oracle 배선(손-oracle 대체) ② 31B self-suggest(자기 불일치→어느 계약줄 박을지 제안) ③ combat 자율 oracle. 곁: eco·combat 잔차 / 외부리뷰 P1(#6·#4·#5·#10). UI/Asset/Renderer는 **별도 트랙**.
