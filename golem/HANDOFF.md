...
1. **(다음 세션 첫 동작) 소설→게임 브리지 첫 실증** — 아뜰리에 초안(`runs/release_manuscript_ko_adversarial_fix_v5/manuscript.md`, "에테르노의 그림자" 12장면)을 골렘 게임으로. 장르 = **내러티브 IF 뼈대 + 턴제 전투 카드(하이브리드, B안 추천)** — 조각5수집·타이머(개기일식 보름)·변칙검술(정석=막힘/변칙=관통, combat_base 규칙으로 직역)·분기 결말. 첫 카드는 "검문 잠입+조각1 각성"을 IF로(detective 패턴 재사용), 이후 변칙 전투를 combat_base로 누적. 골렘이 **구조화(선형→분기 발명)+건전성 검증**, 각색 충실도는 사람. (아뜰리에는 읽기 전용, 수정 금지.)
2. **(목표 핵심 인프라) 싼 사전필터 조각 ① 착수** — 카드 자동 제안(B+검토) `card_proposer.py` 위에, "될 것 같은 top N만 사람에게" 거르는 신호층. 모델-비평가는 불완전한 취향 대리로 명시. 함께 검토 교훈/좋은 예시 레저(RAG) — 사람 검토를 코드로 흡수. (소코반 card4 Boost 제안서 드러난 "자동검사 통과인데 스펙밖" 케이스가 첫 레저 엔트리 후보.)
