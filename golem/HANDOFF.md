...
**현재 핵심(2026-06-18)**: 자율 oracle frontier 종결(G60~64, 루프 닫힘) → 서사 트랙 개시. 로켓 카드로 A겹(서사 발동층) 결정적 검증 입증, reconcile에 자율 oracle 배선 완료(트랙 A). 다음 = 로켓 실빌드로 트랙 C 본선 진입. 방향 정본 = `GolemStudioMode.md` §21(확장)·§6.1(모호성 사전). 아래는 그 이전 §13 파이프라인 배경.
