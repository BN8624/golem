...
11. **(다음, ★키) 자율 oracle 위에서**: ① 고결합 카드(eco/combat) 자율 oracle 정확률 — 결정적 계산 깨지는 난이도 경계(방치형은 산술 단순) ② reconcile 입력에 자율 oracle 배선(손-oracle 대체, 모호탐지→계약fix 루프 자동화) ③ 31B가 자기 불일치 보고 어느 계약줄 박을지 self-suggest. 곁: eco·combat 잔차 / 외부리뷰 P1(#6 taxonomy·#4·#5·#10). UI/Asset/Renderer는 **별도 트랙**(결정적 렌더 채점법 선결).
12. (backlog) levels 등 출력표면 확장 / adversarial validator BLOCKING 추적 / 발열 Adversarial QA·Integration 정식 완주.
