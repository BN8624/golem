...
14. ~~(★키) 로켓 A겹 가능성 프로브~~ — **거의 완료. A겹 작동 입증**: 방치형 로켓 카드(`studio/planning_packet_rocket`+`specqa_packet_rocket`) auto_oracle에서 **events(BEAT-N 발동) 키 전 시나리오 3/3** — 결정적 엔진에 서사 발동층 붙여도 31B가 정확 계산. 부수발견: 첫판 SCN-004/005(WAIT×150=154스텝)가 전키 0.0 붕괴 → **손계산 스케일 한계**(oracle가 머리로 150틱 못 셈, 게임 결함 아님). cost 축소로 18/22스텝 재설계하니 **최종 0.945, events 키 6/6 전부 1.0**(fuel·stage·gameStatus도 1.0). 유일 흠 SCN-003 turn 0.667=한 시드 산술지터(계약 모호 아님). A겹 가능성 입증 완료.
15. **(다음, ★키) 사용자 결정 순서**: 로켓 가능성 확인 → **트랙 A 마무리(=reconcile 배선 하나, combat은 곁)** → 트랙 C(B겹 대사→StoryForge 바이블). **설계 정본 = `GolemStudioMode.md` §21**(큰 게임=결정적 시뮬/전략 / 누적 4레버 / 서사 2겹 / 밸런스 config분리 / 스케일 한계 / 두 트랙), 결정 이유 = context-notes 대화 G. 사람 몫=아트·텍스트 질·재미 취향, 진짜 천장=선택적 컨텍스트. UI/Asset/Renderer는 별도 트랙.
