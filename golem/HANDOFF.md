...
- **▶ 다음 ★키 후보**: ① 누적 빌드 4레버(코드 주입·편집·누적회귀·선택적 컨텍스트, §21.2) — 카드 여러 장 쌓기. ② 룰에 선택/트레이드오프 박은 *재밌는* 카드 재빌드(G65 실플레이 피드백: B겹 서사만으론 밋밋, 게임성은 룰 차원). ③ B겹을 viewer에 연결(BEAT 발동 시 대사 표시, 키0 데모). 설계 정본 = `GolemStudioMode.md` §21. 곁 = combat 자율oracle / 외부리뷰 P1(#6·#4·#5·#10).
