...
- **✓ 외부 코드리뷰 반영 (G71, 2026-06-18, 키0)**: #2 게이트가 첫 시나리오만 종료코드 검사하던 실버그 → 전 시나리오 거부([build_graded.py:230])·#1 FROZEN이 BLOCKING 흡수 수 미확인하던 오판 → 흡수≥질문 수일 때만 동결([planning.py:280]) 둘 다 픽스+키0 회귀잠금. #6 CI = `run_keyless.py`+`.github/workflows/keyless.yml`(push/PR마다 compileall+replay+레버4+게이트+FROZEN, **GitHub Actions 그린 확인**). #3(합의≠정확)·#4(oracle 동일모델)는 골렘 정답앵커가 합의 아닌 **실Node 골든**(모델독립)이라 규율로 정리 — 31B auto_oracle은 모호성 탐지기로만. #5(대형카드)는 아래 스케일 확장과 동일 트랙. 상세 context-notes G71.
- **▶▶ 새 세션 첫 동작 = 4레버 닫힌 뒤 스케일 확장**. 후보: ① 더 큰 카드(모듈 여러개 touched·진짜 컨텍스트 천장 스트레스)로 레버4 한계 측정(=리뷰 #5) ② combat 자율oracle ③ 외부리뷰 P1(#6·#4·#5·#10). 갈림길이라 사용자 선택 대기.
  ```
