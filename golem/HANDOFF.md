...
**현재 상태 (2026-06-19, G78·G79)**: **북극성 두 트랙 동시 전진 — ① 싼 사전필터 닫힘 + ② 소설→게임 브리지 Card1 패킷화.** (G78, ★키 2콜) `card_proposer.py`에 레저(RAG, `cardgen_ledger.py`+`cardgen/`)를 주입하고 사전필터 신호층(static_gate·직전 골든 회귀·발동 커버리지·결정성·구조 스코프)을 붙여 **FLAG·PASS 두 판정 실증**(소코반 card4: 커버리지 1<2 자동 FLAG → 워크드 격자 few-shot 보강 후 Crumbling Floor PASS). 부수발견=산문교훈만으론 31B 격자 좌표 약점 못 막아 few-shot 필요(교훈 종류 차이). (G79, 키0) 아뜰리에 "에테르노의 그림자"(12장면)를 내러티브 IF로 구조화(`bridge_eterno/STRUCTURE.md`) + `eterno_base`(7모듈 IF 엔진) + Card1 패킷(검문잠입+조각1, REF 역산 6시나리오, 회귀 base 골든과 바이트동일). **다음 첫 동작 = `driver_showcase.py eterno` 무인 빌드(★키).** 커밋 7cbccc4·141d138·fc07178·81963f3·3da5e15. (G76 이전 측정 상세는 아래.)
...
1. **소설→게임 브리지 첫 실증 — 구조화+Card1 패킷화 완료(G79), 다음=무인 빌드(★키).** 아뜰리에 "에테르노의 그림자"(12장면, `runs/release_manuscript_ko_adversarial_fix_v5/manuscript.md`, 읽기전용)를 내러티브 IF로 구조화(`bridge_eterno/STRUCTURE.md`: 12장면→노드·4대 메커니즘 직역·선형→분기 발명). `eterno_base`(7모듈 IF 엔진, static_gate 통과) + Card1 패킷(검문잠입+조각1 RULE-08, REF 역산 6시나리오, 회귀 base 골든과 바이트동일, run_keyless ALL PASS). driver에 eterno 등록(l1). 커밋 fc07178·81963f3·3da5e15.
   - **다음 첫 동작 = `python golem/studio/driver_showcase.py eterno`(★키)** — 31B가 base에 Card1(scenes+beats) 패치 → 게이트·합의 1.0·골든 diff 0 그린 판정. detective/sokoban 무인 루프와 동일.
   - 이후: Card2 변칙검술 전투(combat_base 마나방패+ANOMALY 정석0뎀/변칙관통) 누적. 골렘=구조화+건전성, 각색충실도=사람.
