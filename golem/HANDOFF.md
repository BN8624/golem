...
14. ~~(★키) 로켓 A겹 가능성 프로브~~ — **거의 완료. A겹 작동 입증**: 방치형 로켓 카드(`studio/planning_packet_rocket`+`specqa_packet_rocket`) auto_oracle에서 **events(BEAT-N 발동) 키 전 시나리오 3/3** — 결정적 엔진에 서사 발동층 붙여도 31B가 정확 계산. 부수발견: 첫판 SCN-004/005(WAIT×150=154스텝)가 전키 0.0 붕괴 → **손계산 스케일 한계**(oracle가 머리로 150틱 못 셈, 게임 결함 아님). cost 축소로 18/22스텝 재설계하니 SCN-004 1.0 회복. SCN-003 0.67은 한 시드 turn 오집계(events는 3/3). (최종 005/006 마무리 중.)
