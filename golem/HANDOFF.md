...
## ▶ 새 세션 여기부터
- **⚠ IN-FLIGHT (직전 세션 종료 시점, 트랙 C 본선 착수 G65)**: 로켓 실빌드 1단계 `design.py --packet golem/studio/planning_packet_rocket --out golem/studio/design_packet_rocket`(★키)를 백그라운드로 돌려둔 채 세션 종료. **새 세션 첫 동작 = `ls golem/studio/design_packet_rocket/` 확인.** ①폴더 있고 `module_manifest.json`·`traceability.json` 차 있으면 → design 완주, `python golem/studio/design.py --replay`은 아니고 그냥 산출물 검증(module ≥3·BLOCKING 0)하고 다음으로. ②폴더 없거나 비었으면 → 런이 미완/멈춤이니 design.py 재실행(★키, 입력 로딩 픽스는 이미 됨). **배관 픽스 완료**: 로켓 planning 패킷이 프로브용 최소(concept+contract)라 design.py가 요구하는 `acceptance_tests.json`이 없어서, `specqa_packet_rocket/acceptance_tests_draft.json`을 `planning_packet_rocket/acceptance_tests.json`으로 복사함(형식 호환, design.py는 id만 읽음, 6시나리오 SCN-001~006). **다음 ★키 = build_graded** `python golem/studio/build_graded.py --packet golem/studio/planning_packet_rocket --design golem/studio/design_packet_rocket --specqa golem/studio/specqa_packet_rocket --reconcile` → 누적빌드 첫 graded 카드 + reconcile 풀 E2E(Build합의 vs 자율oracle).
...
**현재 핵심(2026-06-18)**: 자율 oracle frontier 종결(G60~64, 루프 닫힘) → 서사 트랙 개시. 로켓 카드로 A겹(서사 발동층) 결정적 검증 입증, reconcile에 자율 oracle 배선 완료(트랙 A). **트랙 C 본선 착수(G65)**: 로켓 실빌드 1단계 design.py 백그라운드 실행 중 세션 종료(위 ⚠ IN-FLIGHT 참조). 방향 정본 = `GolemStudioMode.md` §21(확장)·§6.1(모호성 사전). 아래는 그 이전 §13 파이프라인 배경.
...
16. **(다음, ★키) 트랙 C — 서사 본선**: 로켓 B겹(대사 텍스트, BEAT-N 키에 별도 파일) → StoryForge 일관 바이블. + 누적 빌드 레버(코드 주입·편집·누적회귀·선택적 컨텍스트). **설계 정본 = `GolemStudioMode.md` §21·§6.1(모호성 사전)**, 결정 이유 = context-notes 대화 G. 사람 몫=아트·텍스트 질·재미 취향, 진짜 천장=선택적 컨텍스트. UI/Asset/Renderer는 별도 트랙.
17. (backlog) levels 등 출력표면 확장 / adversarial validator BLOCKING 추적 / 발열 Adversarial QA·Integration 정식 완주.
