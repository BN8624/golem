...
## ▶ 새 세션 여기부터
- 읽는 순서: 이 파일 → **context-notes G62(고결합 자율oracle: 결합 무관·실패는 계약모호)·G61(자율oracle 사다리 0.879→1.0)·G60(자율oracle 프로브 0.879)·G58(Step3 스윕 결판)·G57(외부리뷰 P0)·G55(결합도 가설 기각)·G50(저합의 가드)** → 필요할 때만 `context-notes.md` 나머지(G25~G62) / `GolemStudioMode.md`(설계 정본) / `checklist.md`(진행).
- **지금 할 일 한 줄**: 두 트랙 진전. **(A) 정량 트랙 종결(G53~58)**: 합의를 정하는 건 결합도 아니라 *계약 빡빡함, 단 규칙 종류 의존*(종료조항 올림 d≈3 4회재현 / PHASE 순서 안 올림 / 결합도 가설 기각 eco0.925 vs combat0.633). **(B) 자율 oracle frontier 개시(G60~62) — 현재 라이브 스레드**: 31B가 빌드 0줄로 골든을 자율 생성 → 방치형 0.879, *어휘 한 줄 박으니 1.0 수렴*(G61), 고결합 eco도 결정적 계산은 7/8 무결(G62). **핵심 = 결합도는 계산 안 깨뜨림, 실패는 항상 계약 모호(enum 어휘·번식 규칙)이고 박으면 닫힘 → 자율 oracle은 viable + self-correcting + 공짜 모호탐지기.** 도구 `studio/auto_oracle.py`(키별 `key_accuracy_by_name` 봐야 함, 시나리오단위 0.0은 아티팩트). **다음(★키) = ① eco 어휘+번식 박고 재측정(G61식 사다리) ② reconcile 입력에 자율oracle 배선(손-oracle 대체) ③ 31B self-suggest. 곁 = combat 자율oracle / 외부리뷰 P1(#6·#4·#5·#10)**.
...
12. **(다음, ★키) 자율 oracle 위에서**: ① **eco 어휘+번식 박고 재측정**(G61식 사다리 → entities 8/8·status 8/8 수렴 검증) ② reconcile 입력에 자율 oracle 배선(손-oracle 대체) ③ 31B self-suggest(자기 불일치→어느 계약줄 박을지 제안). 곁: combat 자율 oracle / eco·combat 잔차 / 외부리뷰 P1(#6·#4·#5·#10). UI/Asset/Renderer는 **별도 트랙**.
