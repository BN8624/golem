...
## ▶ 새 세션 여기부터
- 읽는 순서: 이 파일 → **context-notes G63(eco 자율oracle 사다리 1.0 완전수렴·계약버그 탐지)·G62(고결합 자율oracle: 결합 무관·실패는 계약모호)·G61(자율oracle 사다리 0.879→1.0)·G60(자율oracle 프로브 0.879)·G58(Step3 스윕 결판)·G57(외부리뷰 P0)·G55(결합도 가설 기각)·G50(저합의 가드)** → 필요할 때만 `context-notes.md` 나머지(G25~G62) / `GolemStudioMode.md`(설계 정본) / `checklist.md`(진행).
- **지금 할 일 한 줄**: 두 트랙 진전. **(A) 정량 트랙 종결(G53~58)**: 합의를 정하는 건 결합도 아니라 *계약 빡빡함, 단 규칙 종류 의존*(종료조항 올림 d≈3 4회재현 / PHASE 순서 안 올림 / 결합도 가설 기각 eco0.925 vs combat0.633). **(B) 자율 oracle frontier(G60~64) — 루프 닫힘**: 31B가 빌드 0줄로 골든 자율 생성(G60) → 어휘 박으니 1.0(G61) → 고결합 eco도 어휘+번식 박으니 1.0(G63, idle 동형) → **31B가 자기 불일치를 읽고 계약을 자율 처방, 박으니 1.0(G64)**. **생성→탐지→처방→수렴 = self-correcting 완결.** 핵심 = 결합도는 계산 안 깨뜨림(저/고 무관 1.0), 실패는 항상 계약 모호이고 박으면 닫힘. 도구 `studio/auto_oracle.py`·`studio/self_suggest.py`(키별 `key_accuracy_by_name` 봐야 함, 시나리오단위 0.0은 아티팩트). **(C) 서사 레이어 개시 — 현재 라이브**: 큰 게임=결정적 시뮬/전략, 서사 2겹(A겹 발동로직=결정적 golden검증, B겹 텍스트=저작데이터), 텍스트는 출력전용. 방치형 로켓 카드(`planning_packet_rocket`+`specqa_packet_rocket`, A겹=BEAT-N) 준비됨. **다음(★키) = 로켓 auto_oracle(A겹 검증) / reconcile 배선 / combat 자율oracle. 곁 = 외부리뷰 P1(#6·#4·#5·#10)**.
...
14. **(다음, ★키) 두 갈래**. (가) 자율 oracle 생산화: reconcile에 자율 oracle+self-suggest 배선(AUTO/ESCALATE 자리) / combat 자율 oracle. (나) **서사 레이어(라이브)**: 방치형 로켓 카드(`studio/planning_packet_rocket`+`specqa_packet_rocket`, A겹=ADVANCE시 events에 BEAT-N 발동) auto_oracle 검증 → 결정적 엔진에 서사 발동층 붙여도 검증 안 깨지나 첫 증거. 그 위에 B겹(대사 텍스트, 일관 바이블 기반=StoryForge 자리). UI/Asset/Renderer는 별도 트랙.
   - **서사 설계 정본(대화 G)**: 큰 게임=결정적 시뮬/전략(코에이식, 영걸전·삼국지4·심시티·대항해시대4)이 Golem 홈그라운드(eco=아기 심시티, combat=아기 영걸전). 서사는 2겹 — A겹(상태조건 발동 로직=결정적·golden검증) + B겹(대사 텍스트=저작 데이터, 정확일치 안 잼). **규율: 텍스트는 출력전용, 절대 상태로 안 돌아감**(결정성 보존). 일관성은 공유 바이블(StoryForge)에서. 사람 몫=아트·텍스트 질·밸런스, 기술 천장=상태규모(선택적 컨텍스트).
13. (backlog) levels 등 출력표면 확장 / adversarial validator BLOCKING 추적 / 발열 Adversarial QA·Integration 정식 완주.
