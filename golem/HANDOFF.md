...
1. **소설→게임 브리지 첫 실증 — 구조화+Card1 패킷화 완료(G79→G80 A1 재작성), 다음=무인 빌드(★키).** 아뜰리에 "에테르노의 그림자"(12장면, `runs/release_manuscript_ko_adversarial_fix_v5/manuscript.md`, 읽기전용)를 내러티브 IF로 구조화(`bridge_eterno/STRUCTURE.md`: 12장면→노드·4대 메커니즘 직역·선형→분기 발명). **허브형 완결 base**(`eterno_base` 7모듈, start→hub 5조각→대면→결말, 엔진 기계장치 전부 base가 실행, static_gate 통과) + Card1 패킷(검문 잠입 곁가지, **scenes.js만 add-only**, RULE-08, REF 역산 누적 8시나리오, 회귀 base 골든과 바이트동일). 키0 검증기 `_validate_eterno_keyless.py`·run_keyless ALL PASS. driver에 eterno 등록(l1, inject=scenes.js). 커밋 fc07178·81963f3·3da5e15·c194bde·dfabb2b. (G80 전수검사로 1차 스텁 base 설계 결함 잡아 허브형 add-only로 재작성 — 누적 회귀 안전.)
   - **다음 첫 동작 = `python golem/studio/driver_showcase.py eterno`(★키)** — 31B가 base에 Card1(scenes.js) 패치 → 게이트·합의 1.0·골든 diff 0 그린 판정. detective/sokoban 무인 루프와 동일.
