...
- **다음 세션 첫 동작 = `specqa.py` 재런(★키, 프롬프트 fix 효과 확인) → 깨끗하면 `build_graded` 재빌드 → 1.0 목표.** specqa가 또 환각이면 그게 다음 핀/하네스 자리. 1.0 닫히면 그 커널이 **카드 누적의 base**(변칙검술→사거리→지형→유닛→루트). 명령: `python golem/studio/specqa.py --packet golem/studio/planning_packet_tactics_kernel --design golem/studio/design_packet_tactics_kernel --out golem/studio/specqa_packet_tactics_kernel` → 손검산(명령 어휘·REQ-007·출력 shape) → `build_graded.py --packet …_tactics_kernel --design …_tactics_kernel --specqa …_tactics_kernel --out build_runs/tactics_kernel_v4`.
