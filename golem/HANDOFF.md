...
- **▶ 다음 ★키 = 트랙 C 2단계 — 서사 B겹(StoryForge)**: 로켓 카드의 BEAT-N(발동층 A겹 라이브)에 실제 대사 텍스트(B겹, 별도 파일)를 입혀 일관 바이블로. + 누적 빌드 레버(코드 주입·편집·누적회귀·선택적 컨텍스트). 설계 정본 = `GolemStudioMode.md` §21·§6.1. 곁 = combat 자율oracle / 외부리뷰 P1(#6·#4·#5·#10).
