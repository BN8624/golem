...
## ▶ 새 세션 여기부터
- **▶ 트랙 C 본선 다음 동작 (G65, design 완료)**: 로켓 실빌드 1단계 design 완주 — `design_packet_rocket/` 생성됨(4모듈, §7·§8.2 validator **PASS**, DESIGN_STATUS COMPLETE). 리뷰어 BLOCKING=2는 **정보용 카운트지 게이트 아님**(기존 카드 idle 2·combat 3·eco 4·heat 2도 동일하게 비0이고 다 build 진행함 — 핸드오프에 잠깐 "BLOCKING 0" 적었던 건 오기). **새 세션 다음 ★키 = build_graded** `python golem/studio/build_graded.py --packet golem/studio/planning_packet_rocket --design golem/studio/design_packet_rocket --specqa golem/studio/specqa_packet_rocket --reconcile` → 누적빌드 첫 graded 카드 + reconcile 풀 E2E(Build합의 vs 자율oracle). **배관 픽스(완료, 커밋됨)**: 로켓 planning 패킷이 프로브용 최소(concept+contract)라 design.py가 요구하는 `acceptance_tests.json`이 없어서 `specqa_packet_rocket/acceptance_tests_draft.json`을 `planning_packet_rocket/acceptance_tests.json`으로 복사(형식 호환, design.py는 id만 읽음, 6시나리오 SCN-001~006).
