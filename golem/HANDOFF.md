...
**현재 상태 (2026-06-19, G81) — IF 브리지 닫힘 + 장르 피벗(전술 SRPG) + 운영 모델 정형화 + 전술 커널 planning 첫 런(OPEN, 하네스 결함 진단).**
...
- **(★키) 전술 커널 planning 첫 런 → CONTRACT_STATUS OPEN** — `planning.py --synthesize --idea "에테르노 세계관 턴제 전술 그리드…영웅1 이동·인접공격·처치=승리…결정적" --out planning_packet_tactics_kernel`. 골렘 설계 자체는 양호(BLOCKING 19 중 결정11·가정3·보류2 흡수 — 턴사이클·적AI 우선순위·결정적 이동 타이브레이크·충돌·데미지·잘못된입력 다 결정). **OPEN 원인 = 하네스 카운팅 결함**: `planning.py:262` n_block이 리뷰어 10명의 **중복 BLOCKING 19를 그대로 셈** → 흡수 16과 비교(`n_open=19-16=3`). distinct 이슈(~11)는 다 풀렸는데 **중복이 분모를 부풀려 FROZEN 산술 불가.** 골렘 출력 손대지 말고 **게이트 로직을 고치는** 게 정답(첫 "하네스 조이기" 케이스).
- **(키0) 하네스 fix 완료** — `planning.py:262` n_block을 `_dedup`(A/B/C와 동일한 토큰 Jaccard)로 distinct 카운트하게 수정. STATUS에 "원본 N → distinct M" 병기. `_freeze_blocking_keyless.py`에 중복 회귀 4건 추가(중복 7→distinct 2·원본 보존·중복→FROZEN·과대접힘 방지). **실제 전술 패킷 재평가: 원본 19→distinct 16, 흡수 16≥16 → FROZEN 가능**(구조적 불가 해소). run_keyless ALL PASS. (패킷 STATUS.md는 옛 코드가 쓴 OPEN으로 stale — 재런 시 FROZEN으로 갱신.) 더 견고한 버전(synthesis가 unresolved 명시)은 ★키 변경이라 백로그.
- **다음 첫 동작 = `planning.py` 재런(★키) → FROZEN 확인 → `design.py`(★키).** 단계마다 끊어 골렘 출력 검토.
