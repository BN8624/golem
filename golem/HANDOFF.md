...
1. **(다음 세션 첫 동작) 탐정 IF 쇼케이스 확장 — 사용자와 방향 결정** — G75에서 카드 3장 무인 누적 완주(전부 1.0). 후보: ① 카드 더 누적(새 용의자/단서/엔딩 분기 — 단 엔딩 분기는 engine touched라 난이도↑) ② B겹 대사(StoryForge로 장면 텍스트 풍부화, 출력전용·구조만 검증) ③ 밸런스/페이싱 config ④ 또 다른 새 장르. 드라이버(`driver_showcase.py`)에 카드 추가만 하면 무인 확장 가능. (참고: 패치빌드 라인을 §21.2·§21.5 정본과 정합하는 작업도 여기 묶임.)
