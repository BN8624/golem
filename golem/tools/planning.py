...
사용:
  python golem/tools/planning.py --replay <fixture.json>     # 키 안 씀(plumbing 검증)
  python golem/tools/planning.py --idea "..." [--arms A,B,C] # ★키 씀(사용자 go 뒤에만)
...
def _write_outputs(idea, draft, results, api_calls):
    ...
    (BUILD_RUNS / "planning_result.json").write_text(
    ...
    (BUILD_RUNS / "planning_compare.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return summary
...
def main(argv=None):
    ...
    if args.synthesize:
        outdir = Path(args.out) if args.out else (SCRATCH / "planning_packet")
