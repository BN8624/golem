# 전술 카드 l4(유닛) 확장 참조를 키0으로 검증 — 게이트·19세계 골든 일치·결정성 확인
"""★키 빌드 전 안전망. tactics_kernel_base + 확장 참조 game_logic(l3+units) + 계약 세계 주입을
워크스페이스로 만들어 build_graded.gate_and_run(static_gate+contract_validator+node 19시나리오)
...
PACKET = "planning_packet_tactics_l4"
SPECQA = "specqa_packet_tactics_l4"
...
def main():
    ...
    from gen_tactics_l4_golden import REF_GAME_LOGIC
    ...
    ws = HERE / "build_runs" / "_validate_tactics_l4" / "workspace"
    if ws.parent.exists():
