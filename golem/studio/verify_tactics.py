...
전부 PASS여야 그린. 정본 명령: python golem/studio/verify_tactics.py
"""
...
def _card_regression(bg):
    ...
    for lvl in LEVELS:
        pkt = HERE / f"planning_packet_tactics_{lvl}"
        spq = HERE / f"specqa_packet_tactics_{lvl}"
        ...
        ws = HERE / "build_runs" / "_verify_tactics" / lvl
        if ws.exists():
            shutil.rmtree(ws)
    ...
    shutil.rmtree(HERE / "build_runs" / "_verify_tactics", ignore_errors=True)
    return fails
...
def main():
    ...
    stable = HERE / f"tactics_base_{STABLE}"
    promo_fail = None
    ...
    rk = subprocess.run([sys.executable, str(HERE / "run_keyless.py")],
