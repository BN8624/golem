...
def freeze_base(level):
    ...
    mod = reload(mod)
    out = HERE / f"tactics_base_{level}"
    if out.exists():
        shutil.rmtree(out)
    shutil.copytree(HERE / "tactics_kernel_base", out)
...
def main(argv=None):
    ...
    showcase = HERE / "build_runs" / "autocard"
    showcase.mkdir(parents=True, exist_ok=True)
    started = time.time()

    # 시작 base 동결(없으면).
    if not (HERE / f"tactics_base_{args.start}").exists():
    ...
    for level, prev, idea in plan:
        ...
        rc = run([sys.executable, str(HERE / "card_delta.py"), "--level", level, "--prev", prev,
                  "--idea", idea, "--cap", "3"], MAX_SECONDS)
        if rc != 0 or not (HERE / f"planning_packet_tactics_{level}" / "contract.json").exists():
        ...
        rc = run([sys.executable, str(HERE / "build_graded.py"),
                  "--base", str(HERE / f"tactics_base_{prev}"),
                  "--packet", str(HERE / f"planning_packet_tactics_{level}"),
                  "--specqa", str(HERE / f"specqa_packet_tactics_{level}"),
    ...
    if last_good != args.start or not stopped:
        ...
        rc = run([sys.executable, str(HERE / "gen_tactics_story.py"), "--idea", args.setting, "--cap", "3"], MAX_SECONDS)
        story_ok = rc == 0
        rc = run([sys.executable, str(HERE / "gen_tactics_play.py"), "--level", last_good], MAX_SECONDS)
        render_ok = rc == 0
