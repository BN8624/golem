# 전술 카드 l6(상태이상 Corrosion) 확장 참조를 키0으로 검증 — 게이트·25세계 골든 일치·결정성 확인
"""★키 빌드 전 안전망. tactics_kernel_base + 확장 참조 game_logic(l5+corrosion) + 계약 세계 주입을
워크스페이스로 만들어 gate_and_run(static_gate+contract_validator+node 25시나리오) → 골든 대조 + 2회 동일.
engine.js 불변이라 game_logic만 교체.
"""
...
PACKET = "planning_packet_tactics_l6"
SPECQA = "specqa_packet_tactics_l6"
...
def main():
    ...
    ws = HERE / "build_runs" / "_validate_tactics_l6" / "workspace"
    if ws.parent.exists():
