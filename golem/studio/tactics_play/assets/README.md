...
python golem/studio/gen_assets.py sheet --pack roguelike --solid 0.9
# 3) 골렘 비전 선택(★키). --slots=부분집합(생략=전체), 결과는 tile_sprites.json에 병합
python golem/studio/gen_assets.py select --pack roguelike --slots floor,wall,conductive --solid 0.9 --scenario "<테마>"
python golem/studio/gen_assets.py select --pack tiny_dungeon --slots hero,enemy,Hardened,Glass,Resonant --scenario "<테마>"
# 4) 렌더 반영
python golem/studio/gen_tactics_interactive.py --level l9
```
