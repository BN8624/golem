# 전술 카드 l1(마나방패+ANOMALY) 확장 참조를 키0으로 검증 — 게이트·9세계 골든 일치·결정성 확인
"""★키 빌드 전 안전망. tactics_kernel_base + 확장 참조 game_logic + 계약 세계 주입을 워크스페이스로 만들어
build_graded.gate_and_run(static_gate+contract_validator+node 9시나리오) → specqa 골든(expected) 대조
+ 2회 실행 동일(결정성). 빌드가 재현해야 할 참조가 게이트 클린·결정적임을 못박는다.
"""
...
PACKET = "planning_packet_tactics_l1"
SPECQA = "specqa_packet_tactics_l1"
...
def main():
    ...
    from gen_tactics_l1_golden import REF_GAME_LOGIC
