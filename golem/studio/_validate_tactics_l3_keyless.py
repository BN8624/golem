# 전술 카드 l3(지형) 확장 참조를 키0으로 검증 — 게이트·15세계 골든 일치·결정성 확인
"""★키 빌드 전 안전망. tactics_kernel_base + 확장 참조 game_logic(l2+terrain) + 계약 세계 주입을
워크스페이스로 만들어 build_graded.gate_and_run(static_gate+contract_validator+node 15시나리오)
→ specqa 골든(expected) 대조 + 2회 실행 동일(결정성). 빌드가 재현할 참조가 게이트 클린·결정적임을 못박는다.
"""
...
PACKET = "planning_packet_tactics_l3"
SPECQA = "specqa_packet_tactics_l3"
...
def main():
    ...
    from gen_tactics_l3_golden import REF_GAME_LOGIC
    ...
    ws = HERE / "build_runs" / "_validate_tactics_l3" / "workspace"
    if ws.parent.exists():
