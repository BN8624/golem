# 전술 카드 l2(사거리) 확장 참조를 키0으로 검증 — 게이트·12세계 골든 일치·결정성 확인
"""★키 빌드 전 안전망. tactics_kernel_base + 확장 참조 game_logic(l1+ranged) + 계약 세계 주입을
워크스페이스로 만들어 build_graded.gate_and_run(static_gate+contract_validator+node 12시나리오)
...
PACKET = "planning_packet_tactics_l2"
SPECQA = "specqa_packet_tactics_l2"
...
def main():
    ...
    from gen_tactics_l2_golden import REF_GAME_LOGIC
    ...
    ws = HERE / "build_runs" / "_validate_tactics_l2" / "workspace"
    if ws.parent.exists():
