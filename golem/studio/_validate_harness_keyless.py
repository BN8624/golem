...
PKT = HERE / "planning_packet_tactics_l1"
SPQ = HERE / "specqa_packet_tactics_l1"
...
def gate_golden(tag, files):
    ...
    ws = HERE / "build_runs" / f"_harness_{tag}" / "workspace"
    if ws.parent.exists():
...
def make_crash_ws(crash_scn=None):
    ...
    ws = HERE / "build_runs" / "_harness_gate" / "workspace"
    if ws.parent.exists():
        shutil.rmtree(ws.parent)
    ws.mkdir(parents=True)
    write_candidate(ws, files)
...
shutil.rmtree(HERE / "build_runs" / "_harness_selective", ignore_errors=True)
shutil.rmtree(HERE / "build_runs" / "_harness_patch", ignore_errors=True)
shutil.rmtree(HERE / "build_runs" / "_harness_gate", ignore_errors=True)
