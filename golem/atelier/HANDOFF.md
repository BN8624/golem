...
1. **specQA 단계로 진행**(권고, 다음 frontier): design이 양방향(생산 닫힘 + 채점 4축 robust)으로 섰다.
   NovelStudioMode 매핑상 다음은 specQA — FROZEN 아웃라인을 받아 씬별 계약 패킷을 만들고 "검증 불가
   기준=미학으로 격리". 같은 검출/검증 패턴, 같은 생산자(planning/design)+채점기(canon/design_check) 짝 구조.
2. **design 채점기 마지막 축**(키 필요, 한계효용 낮음): 장거리(비트 20+ 누적)만 미시도다. setup충돌은
   사실상 canon_check 범위. 4축을 이미 깼으니 추가 강화 가치는 작다 — 1패스 충분 결론을 굳히는 용도.

> 권고: 1(specQA). design 채점기는 4축 robust로 충분히 닫혔다 — 더 미는 건 한계효용이 낮다. 범위를
> 넓혀 specQA로 가는 게 본류. 보류 카드: 통사적 부정 한계(canon hard3 오탐 0.33→0) — 별도 안건.
