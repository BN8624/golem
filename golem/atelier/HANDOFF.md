...
## 현재 위치 (생산 2 + 채점 2 = 4 도구, planning·design 두 단계 양방향 닫힘)
...
### ▶ 다음 세션 첫 액션 — specQA 단계 (권고)

design이 양방향(생산 닫힘 + 채점 4축 robust)으로 섰고, 외부 리뷰가 짚은 FROZEN 게이트 결함도 고쳐
**계약 동결 토대가 정직해졌다.** 이제 NovelStudioMode 매핑상 다음 frontier는 **specQA**.

- 입력: FROZEN 아웃라인(`outline.json` + `beatsheet.md`)을 계약 패킷으로 받는다.
- 산출: 씬별 계약(scene contract) — 각 씬이 무엇을 충족해야 하나 + 캐논/미학 위험을 **분리**한다
  ("검증 불가한 기준은 미학으로 격리"가 specQA의 핵심 — NovelStudioMode 참조).
- 구조: 지금까지와 같은 짝 — 생산자(planning/design 거울) + 채점기(canon_check/design_check 거울).
  채점 frontier 질문: "31B가 *검증 가능한 씬 기준*과 *미학(검증 불가)*을 옳게 가르나."
- 패턴 재사용: 심은 골든 픽스처 + 키0 replay 배선검증 → ★실콜. 31B 핀+ROLE 가드, 2패스 옵션 그대로.

> 시작점: `NovelStudioMode.md`의 단계 매핑표에서 specQA 행을 먼저 읽고, canon_check.py를 템플릿으로
> `specqa_check.py`부터(채점기 먼저 — canon_check·design_check가 그랬듯). 생산자는 그 뒤.
