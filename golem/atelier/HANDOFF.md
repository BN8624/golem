...
1. **통사적 부정 한계 공략**(작은 카드, 키 필요): 검증 프롬프트에 "부정문/이중부정 방향을 먼저 풀어라"를
   명시하거나 전처리 정규화를 넣어, hard3 subtle_clean 오탐 0.33을 0으로 미는지 본다. 동일 픽스처로 before/after.
   → 풀리면 "통사 한계도 프롬프트로 기계화 가능", 안 풀리면 "부정 구조는 모델 능력 한계" 확정.
2. **design 단계로 진행**(다음 frontier): canon_check이 양방향(거름/안 깎음)으로 닫혔으니 FROZEN 바이블을
   계약 패킷으로 받아 design(비트시트 + setup→payoff)을 얹는다. 같은 검출/검증 2패스 패턴으로 채점.

> 권고: 1은 작은 측정 카드(별도 안건 수준), 2가 본류 진행이다. 부정 한계는 데이터로 확보됐으니 design을
> 먼저 가고 1은 필요할 때 꺼내도 된다. 사용자 판단.
