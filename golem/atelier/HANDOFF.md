...
1. **design 채점기 진짜 한계 찾기**(키 필요): 정황회수·헛회수 두 함정은 1패스가 깼다. 더 센 함정으로 민다 —
   (a) 오회수(엉뚱한 setup을 해결했다 주장), (b) 부분회수(절반만 갚음), (c) 장거리(많은 비트 누적),
   (d) setup끼리 충돌. 여기서 깨지는 지점이 design 채점기의 실거리. 안 깨지면 "setup→payoff는 1패스로 충분" 확정.
...
> 권고: design 채점기가 예상보다 강하다는 신호가 나왔다 — 1로 한 번 더 밀어 진짜 한계를 박으면 경계
> 데이터가 단단해진다. 그게 싫고 범위를 넓히려면 2(specQA). 보류 카드: 통사적 부정 한계(canon hard3
> subtle_clean 오탐 0.33→0) — 별도 안건.
