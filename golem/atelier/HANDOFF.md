...
### ▶ 다음 세션 첫 액션 — specQA ★실콜 (사용자 go 필요)

- **★실콜**(`python specqa_check.py --n 3 --verify`) — 키 사용, **사용자 명시 go 뒤에만**. frontier 질문:
  31B가 격리를 옳게 하나 + 미학→캐논 오라벨(fp)을 2패스가 깎나. canon precision 한계(2패스 필요)의 specQA판인지,
  design처럼 1패스부터 강한지 — 비대칭을 측정.
- **어려운 픽스처** — 미학을 구체어로 위장한 fp 함정("점층한다"처럼 검증돼 보이나 실은 미학) + 캐논을
  부드럽게 쓴 fn 함정. canon/design hard 진행의 specQA판.
