...
- **SQUAD_SELECT**: 제목("출전 부대 편성") + 로스터 유닛 카드 세로 목록(고른 유닛은 강조 테두리/체크) + **예산 표시 "코스트 N/한도"**(초과면 빨강) + 하단 "출전" 버튼(편성 규칙 불만족이면 흐리게/비활성 표시). **각 카드 왼쪽에 유닛 스프라이트 썸네일**(PLAYING과 동일 규칙: `range>1`이면 `tex_mage`, 아니면 `tex_knight` — 적 텍스처 금지), 그 오른쪽에 이름·역할·**cost**·hp/atk. 상세·히트판정은 [★v10]. **히트영역(카드 Rect·출전 버튼)은 [★v10] 그대로 유지**(스프라이트는 표시만 얹는다).
  - **이번 증분(스프라이트 카드) 구체 지시**: 카드 loop 안에서 텍스트를 그리기 전에 썸네일을 추가하라. 예 —
    `var tex = tex_mage if u.get("range",1) > 1 else tex_knight` / `draw_texture_rect(tex, Rect2(rect.position + Vector2(6,9), Vector2(32,32)), false)` 로 16px 스프라이트를 카드 왼쪽에 32x32로 키워 그리고, 텍스트 시작 x를 스프라이트 폭만큼 오른쪽으로 민다(예 `Vector2(46, 30)`). `texture_filter`는 이미 NEAREST. 카드 Rect·출전 버튼 히트영역과 좌표식은 절대 바꾸지 마라.
